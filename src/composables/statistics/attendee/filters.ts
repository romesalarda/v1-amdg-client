...
/**
 * Composable for managing statistics filters
 */
export function useStatisticsFilters(initialEventId?: string) {
  ...
  const queryParams = computed(() => {
    ...
    if (filters.value.age_grouping) {
      params.age_grouping = filters.value.age_grouping
    }
  ...
  // Set age grouping
  const setAgeGrouping = (grouping: 'ranges' | 'individual') => {
    filters.value.age_grouping = grouping
  }
