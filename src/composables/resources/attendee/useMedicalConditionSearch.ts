...
import { useMedicalConditions } from './bookingMedicalConditions'
...
export function useMedicalConditionSearch(searchQuery: MaybeRefOrGetter<string>) {
  ...
  const query = useMedicalConditions(
    computed(() => ({
      page_size: 100,
      ordering: 'label',
  ...
  const options = computed<MedicalConditionOption[]>(() => {
