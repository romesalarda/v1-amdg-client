 ...
 * Returns a complete inventory breakdown for every product in the specified event. For each variant the response includes: current stock, in-flight live order units and their cost, units to reorder (when a max_stock_quantity cap is set), unit price, and the cost to fully restock. Aggregate totals are provided at the product and event level. Intended for end-of-day admin restocking reports.
 */
export const productsInventoryBreakdown2 = <ThrowOnError extends boolean = false>(options: Options<ProductsInventoryBreakdown2Data, ThrowOnError>) => (options.client ?? client).get<ProductsInventoryBreakdown2Responses, ProductsInventoryBreakdown2Errors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }, {
            in: 'cookie',
            name: 'sessionid',
            type: 'apiKey'
 ...
 * Returns a paginated list of attendees who have purchased a specific product variant, enriched with order context (order reference, status, quantity, price). Requires both `event` and `product_variant` parameters.
 */
export const productsInventoryAttendees2 = <ThrowOnError extends boolean = false>(options: Options<ProductsInventoryAttendees2Data, ThrowOnError>) => (options.client ?? client).get<ProductsInventoryAttendees2Responses, ProductsInventoryAttendees2Errors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }, {
            in: 'cookie',
            name: 'sessionid',
            type: 'apiKey'
