  ...
  <div>
    <div class="flex items-center justify-between mb-5">
      <span class="text-sm text-navy-500">
        {{ total > 0 ? `${total} income entr${total !== 1 ? 'ies' : 'y'}` : 'No income entries yet' }}
      </span>
      <button
    ...
    <div v-else-if="debits.length" class="space-y-3">
      <div
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <div class="text-sm font-semibold text-navy-900 truncate">{{ debit.description }}</div>
          <div class="flex items-center gap-2 mt-0.5">
            ...
            <span
              ...
              :class="statusClasses(debit.verification_status)"
            >
              {{ debit.verification_status ?? 'pending' }}
            </span>
            <span v-if="debit.is_settled" class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-green-100 text-green-700">
              Settled
            </span>
          </div>
        </div>
        <div class="text-right flex-shrink-0">
          <div class="text-sm font-black text-green-700">{{ formatMoney(debit.amount) }}</div>
          <div v-if="debit.paid_date" class="text-[10px] text-navy-400 mt-0.5">{{ formatDate(debit.paid_date) }}</div>
        </div>
        <button
          @click="handleDelete(debit.debit_id)"
    ...
    <div v-else class="text-center py-16 text-navy-400">
      <span class="material-symbols-outlined text-5xl block mb-3 opacity-30">arrow_upward</span>
      <p class="text-sm font-semibold">No income entries yet.</p>
      <p class="text-xs mt-1">Record expected income sources for this event.</p>
    </div>
...
import { useDebitExpenses, useDeleteDebitExpense } from '~/composables/resources/payments/debitExpenses'
import CreateDebitExpenseModal from '../modals/CreateDebitExpenseModal.vue'
...
const { data, isLoading } = useDebitExpenses(queryParams)
const debits = computed(() => data.value?.data?.results ?? [])
const total = computed(() => data.value?.data?.count ?? 0)
const totalPages = computed(() => Math.ceil(total.value / PAGE_SIZE))

const deleteMutation = useDeleteDebitExpense()
...
async function handleDelete(debitId: string) {
  ...
  try {
    await deleteMutation.mutateAsync(debitId)
    toast.add({ title: 'Income entry deleted', color: 'green' })
  } catch {
