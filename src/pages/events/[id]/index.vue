<template>
  <div class="min-h-screen bg-white">
    ...
    <!-- Add to Calendar -->
    <div class="relative" ref="calendarMenuRef">
      <button
        @click="showCalendarMenu = !showCalendarMenu"
        class="group w-full flex items-center gap-3 py-3 px-4 border-2 border-deep-navy text-deep-navy rounded-xl hover:bg-deep-navy hover:text-white active:scale-[0.98] transition-all duration-150 font-black text-sm uppercase tracking-wider"
      >
        <span class="w-8 h-8 shrink-0 flex items-center justify-center rounded-lg bg-deep-navy/5 group-hover:bg-white/15 transition-colors">
        ...
        <svg
          ...
          :class="showCalendarMenu ? 'rotate-180' : ''"
      ...
      >
        <div
          ...
          class="absolute top-full mt-2 left-0 right-0 bg-white border border-deep-navy/10 rounded-xl shadow-2xl shadow-deep-navy/10 overflow-hidden z-50 py-1"
        >
          <button
            v-for="cal in calendarOptions" :key="cal.id"
            @click="addToCalendar(cal.id)"
            class="group/item relative w-full flex items-center gap-3 pl-4 pr-4 py-3 text-sm font-bold text-deep-navy hover:bg-mist-blue/20 transition-colors text-left"
          >
            <span class="absolute left-0 top-0 bottom-0 w-0.5 bg-deep-navy scale-y-0 group-hover/item:scale-y-100 transition-transform origin-center"></span>
            <span class="w-6 h-6 flex items-center justify-center shrink-0" v-html="cal.icon"></span>
            {{ cal.label }}
          </button>
        </div>
      </Transition>
    ...
    <div class="relative" ref="shareMenuRef">
      ...
      >
        ...
        <svg
          class="w-4 h-4 ml-auto transition-transform duration-200 ease-out"
          :class="showShareMenu ? 'rotate-180' : ''"
          fill="none" stroke="currentColor" viewBox="0 0 24 24"
        >
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7" />
        </svg>
      ...
      >
        ...
        >
          <button
            v-for="platform in shareOptions" :key="platform.id"
            @click="shareEvent(platform.id)"
            class="group/item relative w-full flex items-center gap-3 pl-4 pr-4 py-3 text-sm font-bold text-deep-navy hover:bg-mist-blue/20 transition-colors text-left"
          >
            <span class="absolute left-0 top-0 bottom-0 w-0.5 bg-deep-navy scale-y-0 group-hover/item:scale-y-100 transition-transform origin-center"></span>
            <span class="w-6 h-6 flex items-center justify-center shrink-0" v-html="platform.icon"></span>
            <span>{{ platform.label }}</span>
            <span v-if="platform.id === 'copy' && linkCopied" class="ml-auto text-emerald-600 text-xs font-black uppercase">Copied!</span>
          </button>
        </div>
      </Transition>
    </div>
...
<svg viewBox="0 0 24 24" fill="none">
  ...
  <text x="12" y="17" text-anchor="middle" font-size="8" font-weight="700" fill="#4285F4">31</text>
</svg>
`,
  },
  {
    ...
    icon: '<svg viewBox="0 0 24 24" fill="currentColor" class="text-gray-800"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>',
  },
  {
    id: 'outlook',
    label: 'Outlook',
...
>
  <!-- Envelope -->
  <path
    d="M9 6H20C20.5523 6 21 6.44772 21 7V17C21 17.5523 20.5523 18 20 18H9V6Z"
    fill="currentColor"
    opacity="0.85"
  />
  <path
  ...
  <!-- O -->
  <path
    d="M7 9.5C5.9 9.5 5.2 10.4 5.2 12C5.2 13.6 5.9 14.5 7 14.5C8.1 14.5 8.8 13.6 8.8 12C8.8 10.4 8.1 9.5 7 9.5Z"
    fill="white"
  />
</svg>
`,
  },
  {
    ...
    icon: '<svg viewBox="0 0 24 24" fill="currentColor" class="text-purple-600"><path d="M19.516 1.366L12 10.25 4.484 1.366H0l8.871 10.91L0 22.634h4.484L12 13.75l7.516 8.884H24l-8.871-10.358L24 1.366h-4.484z"/></svg>',
  },
  {
    id: 'ical',
    label: 'Download .ics file',
    icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-deep-navy"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>',
  },
]
...
const shareOptions = [
  {
    id: 'copy',
    label: 'Copy Link',
    icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-deep-navy"><path stroke-linecap="round" stroke-linejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>',
  },
  {
    id: 'twitter',
    label: 'Share on X (Twitter)',
    icon: '<svg viewBox="0 0 24 24" fill="currentColor" class="text-black"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>',
  },
  {
    id: 'facebook',
    label: 'Share on Facebook',
    icon: '<svg viewBox="0 0 24 24" fill="currentColor" class="text-blue-600"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>',
  },
  {
    id: 'whatsapp',
    label: 'Share on WhatsApp',
    icon: '<svg viewBox="0 0 24 24" fill="currentColor" class="text-green-500"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>',
  },
  {
    id: 'telegram',
    label: 'Share on Telegram',
    icon: '<svg viewBox="0 0 24 24" fill="currentColor" class="text-sky-500"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>',
  },
  {
    id: 'email',
    label: 'Share via Email',
    icon: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-deep-navy"><path stroke-linecap="round" stroke-linejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>',
  },
]

const formatIcsDate = (dateStr?: string): string => {
  if (!dateStr) return ''
  return dateStr.replace(/[-:]/g, '').replace('.000Z', 'Z').split('.')[0] + 'Z'
}

const buildIcsContent = (): string => {
  const e = event.value
  if (!e) return ''
  const location = primaryVenue.value
    ? [primaryVenue.value.address, primaryVenue.value.city].filter(Boolean).join(', ')
    : ''
  const description = (e.short_description || '').replace(/\n/g, '\\n')
  return [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//AMDG//Event//EN',
    'BEGIN:VEVENT',
    `UID:event-${e.event_id}@amdg`,
    `DTSTAMP:${formatIcsDate(new Date().toISOString())}`,
    `DTSTART:${formatIcsDate(e.start_datetime)}`,
    `DTEND:${formatIcsDate(e.end_datetime)}`,
    `SUMMARY:${e.title}`,
...
const addToCalendar = (type: string) => {
  ...
  const start = formatIcsDate(e.start_datetime)
  const end = formatIcsDate(e.end_datetime)
  const location = primaryVenue.value
    ? encodeURIComponent([primaryVenue.value.address, primaryVenue.value.city].filter(Boolean).join(', '))
    : ''
  const details = encodeURIComponent(e.short_description || '')

  if (type === 'google') {
    const url = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${title}&dates=${start}/${end}&details=${details}&location=${location}`
    window.open(url, '_blank', 'noopener,noreferrer')
  } else if (type === 'outlook') {
    const url = `https://outlook.live.com/calendar/0/deeplink/compose?subject=${title}&startdt=${encodeURIComponent(e.start_datetime || '')}&enddt=${encodeURIComponent(e.end_datetime || '')}&body=${details}&location=${location}`
    window.open(url, '_blank', 'noopener,noreferrer')
  } else if (type === 'yahoo') {
    const url = `https://calendar.yahoo.com/?v=60&title=${title}&st=${start}&et=${end}&desc=${details}&in_loc=${location}`
    window.open(url, '_blank', 'noopener,noreferrer')
  } else if (type === 'apple' || type === 'ical') {
...
const shareEvent = async (platform: string) => {
  ...
  // If Web Share API is available and it's a mobile/native share, try it first
...
const handleClickOutside = (e: MouseEvent) => {
  if (showCalendarMenu.value && calendarMenuRef.value && !calendarMenuRef.value.contains(e.target as Node)) {
    showCalendarMenu.value = false
  }
  if (showShareMenu.value && shareMenuRef.value && !shareMenuRef.value.contains(e.target as Node)) {
    showShareMenu.value = false
  }
}
